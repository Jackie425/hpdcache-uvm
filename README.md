...
The `hpdcache_atomic_test` uses the same test/virtual-sequence/worker layering as
the default test and focuses traffic on atomic operations. Each active worker
generates LR through the AMO API, then constructs the matching SC in the LRSC
worker and copies the LR address, size, byte enable, and PMA. It mixes those
pairs with AMO API sequences whose opcode is generated by the API's own
constraints; intervening LR/SC traffic may also contain ordinary operations.
Ordinary AMO requests continue to select cacheable and uncacheable regions
...
The current random regression uses the item's soft distribution for LOAD, STORE,
all AMO operations, and all CMO operations. The memory response model runs with
LIGHT random backpressure; ordinary and AMO error injection remains disabled for
this phase. Active prefetch traffic, IO accesses, error campaigns, functional
coverage, and custom SVA remain outside this feature.
